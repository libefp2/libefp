...
def test_efpefptorque():
    ...
    assert compare_values(-0.0066095987170644, ene['total'], sys._getframe().f_code.co_name + ': ene', atol=1.e-3)
    assert compare_recursive(ref, {'torque': torq}, sys._getframe().f_code.co_name + ': torq', atol=1.e-3)
...
def test_efpefp_bz2():
    ...
    assert compare_values(-0.006945881265, ene['elst'], sys._getframe().f_code.co_name + ': ene elst', atol=1.e-3)
    assert compare_values(0.046915489574, ene['exch'], sys._getframe().f_code.co_name + ': ene exch', atol=1.e-3)
    assert compare_values(-0.000675030191, ene['ind'], sys._getframe().f_code.co_name + ': ene ind', atol=1.e-3)
    assert compare_values(-0.021092526180, ene['disp'], sys._getframe().f_code.co_name + ': ene disp', atol=1.e-3)
    assert compare_values(0.018202051938, ene['total'], sys._getframe().f_code.co_name + ': ene', atol=1.e-3)
