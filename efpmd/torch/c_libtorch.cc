...
void ANIModel::get_energy_grad(const torch::Tensor& coordinates, 
    ...
    //auto atomic_energies_tensor = module.get_method("atomic_energies")(inputs).toTuple()->elements()[1].toTensor().to(torch::kDouble);
    //auto aten_array = atomic_energies_tensor.accessor<double, 2>();
	...
	//for (int i = 0; i < aten_array.size(1); i++) {
        //   double atomic_energy_value = aten_array[0][i]; // Access the first batch
        //   std::cout << "Atom " << i << ": " << std::fixed << std::setprecision(12) << atomic_energy_value << std::endl; // Print the value
        //}
    }       
...
/* 
//previous version with float custom energy
void ANIModel::get_custom_energy_grad(float* coordinates_data, int64_t* species_data, float* elecpots_data, int num_atoms, float* custom_energy, float* cus_grads, float* cus_forces, int print) {
...
// correct version with double custom energy
...
//        };
    ...
    std::map<int, double> ani1x_sae_dict_byIdx = {
                {0, -0.6052763339909124}, // H
                {1, -38.08693749320107}, // C
                {2, -54.70947532472019}, // N
                {3, -75.19767366500193} // O
        };
...
//    memcpy(custom_energy, energy_shifted.data_ptr<double>(), sizeof(double));
}
...
void get_ani_energy_grad(ANIModel* model, float* coordinates, int* species, double* atomic_energies, float* gradients, float* forces, int num_atoms, int print) {
    ...
    model->get_energy_grad(coordinates_tensor, species_tensor, atomic_energies, gradients, forces, num_atoms, print);

}


/*
void get_ani_energy_grad(ANIModel* model, double* coordinates, int* species, double* atomic_energies, double* gradients, double* forces, int num_atoms, int print) {

    auto coordinates_tensor = torch::from_blob((double*)coordinates, {1, num_atoms, 3}, torch::requires_grad(true));
    auto species_tensor = torch::from_blob((int*)species, {1, num_atoms}, torch::kInt32);

    std::cout << "In c_libtorch.cc, inside get_ani_energy_grad()" << std::endl;
    model->get_energy_grad(coordinates_tensor, species_tensor, atomic_energies, gradients, forces, num_atoms, print);
